...
// UPDATE SETS
router.put("/update/:id", verifySetAuthor, async (req, res) => {
    try {
...
// DELETE SETS
router.delete("/delete/:id", verifySetAuthor, async (req, res) => {
    try {
      await Set.findByIdAndDelete(req.params.id);
      res.status(200).json("The sets has been deleted...");
    } catch (err) {
      res.status(500).json(err);
    }
});
  ...
  try {
    ...
    for (let i in sets) {
      for (let j in sets[i].cards) {
        if (sets[i].cards[j].level !== 0) {
          cardsLearn = cardsLearn.concat(sets[i].cards[j])
        }
      }
    }
    res.status(200).json(cardsLearn);
  } catch (err) {
    res.status(500).json(err)
  }
});
...
router.get("/review", verifyToken, async (req, res) => {
  try {
    ...
    for (let i in sets) {
      for (let j in sets[i].cards) {
        const cardDate = sets[i].cards[j].updatedAt.getUTCDate();
        const nowDate = new Date().getUTCDate();
        if (sets[i].cards[j].level === 2 && (nowDate - cardDate) > 2 ) {
          cardsReview = cardsReview.concat(sets[i].cards[j])
        }
        else if (sets[i].cards[j].level === 1 && (nowDate - cardDate) > 0 ) {
          cardsReview = cardsReview.concat(sets[i].cards[j])
        }
      }
    }
